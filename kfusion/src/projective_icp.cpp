...
bool
kfusion::cuda::ProjectiveICP::estimateTransform(Affine3f & /*affine*/, const Intr & /*intr*/, const Frame & /*curr*/,
                                                const Frame & /*prev*/) {
...
bool kfusion::cuda::ProjectiveICP::estimateTransform(Affine3f &affine, const Intr &intr, const PointsPyr &vcurr,
    ...
    for (int level_index = LEVELS - 1; level_index >= 0; --level_index) {
        const auto &n = (const device::Normals &) nprev[level_index];
        const auto &v = (const device::Points &) vprev[level_index];
